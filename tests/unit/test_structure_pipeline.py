        ...
        return [
            {
                **rows[0],
                ...
                "harvest_reason": "No source structure payload.",
            },
            {
                **rows[1],
                "harvest_status": "conflict",
                "harvest_reason": "Conflicting InChIKey.",
            },
            {
                **rows[2],
...
def test_structure_pipeline_handles_preclassified_unmatched_conflict_and_error(monkeypatch):
    monkeypatch.setattr("herg.structure_pipeline.get_conn", lambda **kwargs: _DummyConnection())
    monkeypatch.setattr("herg.structure_pipeline.ensure_structure_enrichment_schema", lambda cur: None)
    ...
    stats = run_structure_pipeline(
        ...
        StructureRunConfig(dry_run=True),
    )
