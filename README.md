...
This repository contains a small SQL-first application for storing, enriching, and browsing hERG IC50 measurements. It uses PostgreSQL for the canonical data model, Streamlit for manual entry and browsing, and Python pipeline scripts for ChEMBL, PubChem, UniChem, identifier, and structure enrichment workflows.

The database keeps compound identity, source provenance, and IC50 measurements normalized. `ic50_um`, `pic50`, and `pic50_qualifier` are generated by PostgreSQL rather than application code.
...
Use the Streamlit UI for manual entry, CSV upload, browsing, dashboard views, and CSV export. The upload tab includes CSV templates for compounds and IC50 results.
